...
<main>
  ...
  <form class="hidden" id="configForm">
    ...
    <section id="openMeteoSettings">
      <h2>Open-Meteo</h2>
      <div class="grid">
        <div class="span-7"><label for="openMeteoCity">Město</label><input id="openMeteoCity" name="openMeteoCity" maxlength="63" value="Brno" placeholder="Brno"><input id="openMeteoLatitude" name="openMeteoLatitude" type="hidden"><input id="openMeteoLongitude" name="openMeteoLongitude" type="hidden"></div>
    ...
    <section>
      ...
      <div class="grid">
        ...
        <div class="span-6"><div class="switch-row"><span class="field-label">Úvodní nula hodiny</span><label class="switch" aria-label="Zobrazit úvodní nulu hodiny"><input id="showLeadingHourZero" name="showLeadingHourZero" type="checkbox"><span></span></label></div><span class="hint">Například 08:15 místo 8:15.</span></div>
      </div>
    </section>
    <section>
      <h2>Vteřiny</h2>
      <div class="grid">
        <div class="span-4"><label for="timeColonEffect">Dvojtečka hodin</label><select id="timeColonEffect" name="timeColonEffect"><option value="steady">Neblikat</option><option value="blink">Blikat</option><option value="fade">Pohasínat</option></select><span class="hint">Blikání je bez přechodu, pohasínání je plynulé.</span></div>
        <div class="span-4"><label for="secondEffect">Efekt vteřin</label><select id="secondEffect" name="secondEffect"><option value="off">Vypnuto</option><option value="dots">Klasické tečky</option><option value="line">Plynulá čára</option><option value="comet">Kometa</option></select><span class="hint">Vteřiny lze přepnout také přímo na hodinách.</span></div>
...
function updateDataSource(){const open=$("dataSource").value==="open-meteo";$("openMeteoSettings").classList.toggle("hidden",!open);document.querySelectorAll(".ha-only").forEach(section=>section.classList.toggle("hidden",open));updateAutomaticDayNightControls()}
...
$("configForm").addEventListener("submit",async event=>{event.preventDefault();if($("dataSource").value==="home-assistant"&&$("automaticDayNight").checked&&!$("sunEntity").value.trim()){$("saveFeedback").className="feedback error";$("saveFeedback").textContent="Pro automatický režim DEN/NOC musí být vyplněna SUN entita.";$("sunEntity").focus();return}const button=$("saveButton");button.disabled=true;$("saveFeedback").className="feedback";$("saveFeedback").textContent="Ukládám…";const data=Object.fromEntries(new FormData(event.currentTarget).entries());["leftName","rightName"].forEach(field=>data[field]=(data[field]||"").toLocaleUpperCase("cs-CZ"));["metricA","metricB"].forEach(prefix=>{if(data[prefix+"Name"])data[prefix+"Name"]=data[prefix+"Name"].toLocaleUpperCase("cs-CZ")});openMeteoSlotTitles.forEach((_,index)=>{const field=`openMeteoSlot${index}Name`;data[field]=(data[field]||"").toLocaleUpperCase("cs-CZ")});data.automaticDayNight=$("automaticDayNight").checked?"1":"0";data.automaticFirmwareUpdate=$("automaticFirmwareUpdate").checked?"1":"0";data.animatedWeatherIcons=$("animatedWeatherIcons").checked?"1":"0";data.weatherIconStyle=$("weatherIconStyle").value||"monochrome";data.showLeadingHourZero=$("showLeadingHourZero").checked?"1":"0";try{await request("/api/config",data);if(data.webMode!=="disabled")await loadConfig();$("haToken").value="";$("tokenState").textContent="Token je uložen";$("saveFeedback").className="feedback success";$("saveFeedback").textContent=data.webMode==="disabled"?"Nastavení bylo uloženo a webový server byl vypnut. Znovu jej můžeš povolit na displeji hodin.":"Nastavení bylo úspěšně uloženo."}catch(error){$("saveFeedback").className="feedback error";$("saveFeedback").textContent=error.message}finally{button.disabled=false}});
