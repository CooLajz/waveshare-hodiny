...
await page.evaluate(()=>{document.getElementById('dataSource').value='home-assistant';updateDataSource();document.getElementById('automaticDayNight').checked=false;});
await page.locator('#forecastTemperatureEntity').fill('sensor.forecast_temperature');
await page.evaluate(()=>saveConfiguration());
assert.equal(config.forecastTemperatureEntityId,'sensor.forecast_temperature');
assert.equal(await page.locator('#forecastTemperatureEntity').inputValue(),'sensor.forecast_temperature');
...
await page.locator('#notificationMessage').fill('První řádek\nDruhý "řádek"');
await page.locator('#testNotification').click();
await page.waitForFunction(()=>!document.getElementById('testNotification').disabled);
assert.equal(notifications.length,1);assert.equal(saves,savedCount);
assert.deepEqual(JSON.parse(await page.locator('#notificationJson').inputValue()),notifications[0]);
assert.equal(notifications[0].message,'První řádek\nDruhý "řádek"');
assert.equal(notifications[0].beep,150);assert.equal(notifications[0].durationSeconds,15);
assert.equal(await page.locator('#notificationUrl').inputValue(),origin+'/api/control/test-notification-secret/notification');
assert((await page.locator('#notificationFeedback').innerText()).includes('odeslána'));
await page.locator('#notificationTester').screenshot({path:'/private/tmp/notification-web-desktop.png',style:'header,.tab-nav,.footer{visibility:hidden!important}'});
await page.locator('#notificationBeep').fill('5001');await page.locator('#testNotification').click();
assert.equal(notifications.length,1);
assert.equal(await page.locator('#notificationBeep').evaluate(el=>el.form.id),'notificationTestForm');
assert.equal(await page.locator('#configForm').evaluate(el=>new FormData(el).has('notificationBeep')),false);
await page.locator('#notificationBeep').fill('0');await page.locator('#notificationSeconds').fill('0');
notificationStatus=409;await page.locator('#testNotification').click();await page.waitForFunction(()=>!document.getElementById('testNotification').disabled);
assert((await page.locator('#notificationFeedback').innerText()).includes('vypnutý'));
assert.equal(notifications.at(-1).durationSeconds,0);assert.equal(notifications.at(-1).beep,0);
notificationStatus=200;await page.locator('#testNotification').click();await page.waitForFunction(()=>!document.getElementById('testNotification').disabled);
await page.evaluate(()=>{Object.defineProperty(navigator,'clipboard',{value:undefined,configurable:true});window.copiedNotification='';document.execCommand=command=>{if(command==='copy')window.copiedNotification=document.activeElement.value;return true}});
await page.locator('#copyNotificationJson').click();assert.equal(await page.evaluate(()=>window.copiedNotification),await page.locator('#notificationJson').inputValue());
await page.locator('#copyNotificationUrl').click();assert.equal(await page.evaluate(()=>window.copiedNotification),await page.locator('#notificationUrl').inputValue());
await page.setViewportSize({width:390,height:844});
await page.locator('#notificationTester').screenshot({path:'/private/tmp/notification-web-mobile.png',style:'header,.tab-nav,.footer{visibility:hidden!important}'});
assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
await page.evaluate(()=>applyDeviceLanguage('en'));assert.equal(await page.locator('label[for="notificationMessage"]').innerText(),'Message');
console.log('Notification form, JSON POST, multiline text, validation, error recovery, copy fallback and mobile layout: OK');
